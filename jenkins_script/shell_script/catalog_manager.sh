...
function restore_catalog() {
  ...
  if [[ -d ${backup_dir} && -f ${schema_content} ]]; then
    ...
    gsql "use graph computerNet ls" > /tmp/schema_content
    # compare current schema content with old schema content which is expected to be included
    echo '-------------------------base------------------'
    cat ${schema_content}
    echo '-------------------------base------------------'
    echo '-------------------------output------------------'
    cat /tmp/schema_content
    echo '-------------------------output------------------'

    python ${PYTHON_SCRIPT_FOLDER}/compare_file.py ${schema_content} /tmp/schema_content
  else
